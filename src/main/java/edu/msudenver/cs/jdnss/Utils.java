...
class Utils {
     ...
     */
    static byte[] toCS( final String s) {
        if (s == null) {
     ...
     * @return the converted form in bytes
     */
    static byte[] convertString( final String s) {
        if (s == null) {
            throw new java.lang.NullPointerException("s is marked non-null but is null");
        }
        assert !s.equals("");
    ...
    static byte[] trimByteArray( final byte[] old, final int length) {
        if (old == null) {
            throw new java.lang.NullPointerException("old is marked non-null but is null");
        }
     ...
     */
    static String reverseIP( final String s) {
        if (s == null) {
            throw new java.lang.NullPointerException("s is marked non-null but is null");
        }
        String[] a = s.split("\\.");
     ...
     */
    static int count( final String s,  final String c) {
        if (s == null) {
            throw new java.lang.NullPointerException("s is marked non-null but is null");
        }
        if (c == null) {
            throw new java.lang.NullPointerException("c is marked non-null but is null");
        }
    ...
    static Map.Entry<String, Integer> parseName(int start, byte[] buffer) {
        ...
        if ((buffer[current] & 192) == 192) {
            current = getCurrent(start, buffer, current, name);
        }
        int length = buffer[current++] & 63;
        if (length == 0) {
            return Map.entry("", 1);
        }
        while (length > 0) {
    ...
    private static int getCurrent(int start, byte[] buffer, int current, StringBuilder name) {
        int tmp = addThem(buffer[current] & 63, buffer[current + 1]);
        logger.trace(tmp);
        if (tmp >= start) {
            logger.warn("Illegal name");
            assert false;
